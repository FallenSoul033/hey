...
let data={orders:[],clients:[],production:[],employees:[],accruals:[]};

const navAll=[['dashboard','⌂','Обзор'],['orders','▤','Заказы'],['clients','♙','Клиенты'],['production','❄','Производство'],['employees','♧','Сотрудники'],['accruals','₸','Начисления'],['warehouse','▦','Склад'],['analytics','↗','Аналитика']];
const titles={dashboard:['Обзор','Панель управления'],orders:['Продажи','Заказы'],clients:['CRM','Клиенты'],production:['Операции','Производство'],employees:['Команда','Сотрудники'],accruals:['Оплата труда','Начисления'],warehouse:['Остатки','Склад'],analytics:['Показатели','Аналитика']};
...
function showOnly(target){$('#auth-screen').hidden=target!=='auth';$('#onboarding').hidden=target!=='onboarding';document.body.classList.toggle('app-ready',target==='app')}
...
function resetIdentity(){stopRealtime();session=null;profile=null;organization=null;manager=false;appLoaded=false;hydratedUserId=null;hydratingUserId=null;data={orders:[],clients:[],production:[],employees:[],accruals:[]}}
...
async function init(){
  ...
  try{
    ...
    supabase.auth.onAuthStateChange((event,next)=>{
      session=next;
      if(!next){resetIdentity();replaceRoute('login');setTimeout(applyRoute,0);return}
...
async function loadAll(){
  ...
  data.accruals=manager?results[5].data.map(x=>({id:x.id,date:x.accrual_date,employeeId:x.employee_id,employee:x.employee_name,description:x.description,qty:Number(x.quantity),rate:Number(x.rate),paid:x.paid})):[];
  ...
  setSync('Синхронизировано','ok');return true;
}

function subscribe(){stopRealtime();realtime=supabase.channel(`icefresh-${profile.organization_id}`);for(const tableName of ['clients','employees','orders','production_entries',...(manager?['accruals']:[])])realtime.on('postgres_changes',{event:'*',schema:'public',table:tableName,filter:`organization_id=eq.${profile.organization_id}`},scheduleRefresh);realtime.subscribe(status=>setSync(status==='SUBSCRIBED'?'Онлайн':'Подключение…',status==='SUBSCRIBED'?'ok':''))}
...
function buildNav(){const allowed=manager?navAll:navAll.filter(x=>!['employees','accruals','analytics'].includes(x[0]));$('#nav').innerHTML=allowed.map(n=>`<button data-section="${n[0]}"><span>${n[1]}</span>${n[2]}</button>`).join('')}
...
const views={dashboard,orders,clients,production,employees,accruals,warehouse,analytics};
function render(){if(!views[section])section='dashboard';const t=titles[section];$('#eyebrow').textContent=t[0];$('#title').textContent=t[1];$('#app').innerHTML=views[section]();document.querySelectorAll('#nav button').forEach(b=>b.classList.toggle('active',b.dataset.section===section));$('#add').hidden=['dashboard','warehouse','analytics'].includes(section)||(!manager&&['employees','accruals'].includes(section));document.querySelector('.sidebar').classList.remove('open')}
...
async function createInvite(){const {data:invite,error}=await supabase.from('organization_invites').insert({organization_id:profile.organization_id,role:'staff',created_by:session.user.id}).select('token,expires_at').single();if(error){toast(friendlyError(error));return}let copied=false;try{if(navigator.clipboard?.writeText){await navigator.clipboard.writeText(invite.token);copied=true}}catch{}window.prompt('Передайте этот одноразовый код сотруднику:',invite.token);toast(copied?'Код приглашения скопирован':'Код приглашения создан')}
...
$('#app').onclick=e=>{const target=e.target.closest('[data-go]');if(target){go(target.dataset.go);return}if(e.target.closest('#invite'))createInvite()};
